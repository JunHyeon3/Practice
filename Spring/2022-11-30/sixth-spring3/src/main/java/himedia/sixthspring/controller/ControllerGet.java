...
//	}
	...
	/** [parameter에서 사용되는 @ModelAttribue와 Model의 차이]
	 *  @ModelAttribute
	 *  - view -> controller
	 *  - view에서 controller로 데이터가 넘어올 때 사용
	 *  - 주로 수정, 등록에 사용
	 *  
	 *  Model
	 *  - controller -> view
	 *  - controller에서 view로 데이터를 넘겨줄 때 사용
	 *  - model.addAttribute를 사용하며, 생략이 가능하다.
	 */
	@GetMapping("/cancel")
	public String cancel(@ModelAttribute Member member, Model model) {
		...
		// [생략 가능]
		// 생략하면 model.addAttribute(member)형태로 자동으로 저장되어 View로 넘겨준다.
